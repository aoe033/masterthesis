...
\begin{tikzpicture}[thick]
  ...
  \node (wait) [process, below right = 3cm of tx] {wait (delay)};

  \node (rx) [cycle, below left = 3cm of wait] {retrieve byte \\ from COM};
  \node (reqwrite) [ann, below right = 1cm of rx, xshift=-5.0em] {\uline{$data + adr(0x00 - 0x41)$}};

  \node (dec) [decision, above left = 2cm of rx] {$adr < 0x41$};
  \node (incr) [process, above = 1cm of dec] {adr++};
  ...
  \path [arrow] (tx) edge [in=90,out=0,above,distance=2cm] node[align=center] {} (wait);      

  \path [arrow] (wait) edge [in=0,out=270,above,distance=2cm] node[align=center] {} (rx);  

  \path [arrow] (rx) edge [in=270,out=180,above,distance=1cm] node[align=center] {} (dec); 
