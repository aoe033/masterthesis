...
\tikzstyle{fpgas} = [PCs, text width=5em, fill=black!10, rounded corners, 
    minimum height=12em, minimum width=15em]
\def\blockdist{5.3}
\def\btmdist{-0.7}
\def\topdist{1.0}
\def\edgedist{2.5}
...
\begin{tikzpicture}
    ...
    \path (fpga)+(-1.3,\btmdist) node (uart) [PCs] {UART};
    \path (fpga)+(-1.3,\topdist) node (dec) [PCs] {UART decoder};

    \path (fpga)+(1.3,\topdist) node (top) [PCs] {UART top};
    \path (fpga)+(1.3,\btmdist) node (gbt) [PCs] {GBT example};
    ...
    \path (fpga.south east)+(-2.7,0.4) node (FPGA) {FPGA};

    %\node (FPGA) [below of=uart] {FPGA};

    \path (fpga.south west)+(0.8,-0.4) node (INS) {PC to CRU serial interface};
    ...
    \begin{pgfonlayer}{background}
        ...
        \path[fill=blue!10,rounded corners, draw=black]
            (a) rectangle (b);
    \end{pgfonlayer}
\end{tikzpicture}
