...
ncurses is a terminal control library commonly used with Unix-systems. The library enables you to construct interfaces using the terminal as the base. The main purpose of using ncurses for the software interface was to enable multiple windows; one window to continuously update and display the \gls{gbt} control signals, and another window for the user to write commands. To compile ncurses-based programs in Windows, one can use the \textit{pdcurses} library instead, which has the same function names and overall functionality, so no alterations of code is needed.
